...
#if defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR1100) || defined(CONFIG_WNR612) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3) || defined(CONFIG_WNR1000V4)
...
static void ag7240_get_ethaddr(struct eth_device *dev)
{
    ...
    /* Modified by Micu, MAC does not need to begin */
    /* with 0x00 to be valid. */
...
#if (CONFIG_COMMANDS & CFG_CMD_MII)
int ag7240_miiphy_read(char *devname, uint32_t phy_addr, uint8_t reg, uint16_t *value)
{
...
int ag7240_miiphy_write(char *devname, uint32_t phy_addr, uint8_t reg, uint16_t data)
{
    ag7240_mac_t *mac = ag7240_name2mac(devname);
