...
NetLoop(proto_t protocol)
{
	...
	eth_halt();
#ifdef CONFIG_NET_MULTI
#if defined(CFG_VITESSE_73XX_NOPHY) || defined(CONFIG_WNR1000V2) || defined(CONFIG_WNR1100) || defined(CONFIG_WNR612) || defined(CONFIG_WNR2200) || defined(CONFIG_WNR2000V3)
	/*
...
void NetStartAgain (void)
{
	...
	NetSetTimeout (5 * CFG_HZ, startAgainTimeout);
	...
	case PROT_IP:
		...
		if (ip->ip_p == IPPROTO_ICMP) {
			...
			case ICMP_ECHO_REQUEST:
				...
				memcpy((void *)&PingOff,(void *)&(ip->ip_off),sizeof(ushort));
				MyPingReply();
				return;
#endif
			case ICMP_REDIRECT:
...
void update_firmware(ulong addr, int firmware_size)
{
	...
	for (offset_num = 0;
	     offset_num < ((firmware_size / CFG_FLASH_SECTOR_SIZE) + 1);
	     offset_num++) {
		src_addr = (uchar *)(addr + offset_num * CFG_FLASH_SECTOR_SIZE);
